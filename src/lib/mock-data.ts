// ============================================================
// Tandot — Realistic seed / mock data for demo
// ============================================================

import type {
  Tanda,
  TandaMember,
  Contribution,
  Payout,
  DashboardStats,
} from './types';

// ── Dashboard Stats ─────────────────────────────────────────
export const MOCK_STATS: DashboardStats = {
  total_tandas: 47,
  active_tandas: 12,
  total_volume_mxn: 2_847_500,
  total_members: 156,
  avg_trust_score: 82,
  successful_payouts: 234,
  fraud_prevented: 3,
};

// ── Tandas ──────────────────────────────────────────────────
export const MOCK_TANDAS: Tanda[] = [
  {
    id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    name: 'Las Comadres de Puebla',
    description: 'Tanda semanal entre amigas del mercado de La Victoria. Ahorramos para Navidad. 🎄',
    contribution_amount: 500,
    frequency: 'weekly',
    max_members: 10,
    current_round: 4,
    total_rounds: 10,
    status: 'active',
    escrow_address: '0x9c400171c76F1eaE1D64aD98E1a0bFB8B871BbdE',
    ai_trust_score: 94,
    created_at: '2026-04-15T10:00:00Z',
    next_payout_at: '2026-05-18T10:00:00Z',
  },
  {
    id: 'd2885ba1-1a2d-4af4-8d80-afc732c33c26',
    name: 'Albañiles de Nezahualcóyotl',
    description: 'Tanda quincenal de la cuadrilla. Para herramientas y emergencias. 🔨',
    contribution_amount: 1000,
    frequency: 'biweekly',
    max_members: 8,
    current_round: 2,
    total_rounds: 8,
    status: 'active',
    escrow_address: '0x8ba1f109551bD432803012645Ac136ddd64DBA72',
    ai_trust_score: 87,
    created_at: '2026-04-20T14:00:00Z',
    next_payout_at: '2026-05-20T14:00:00Z',
  },
  {
    id: '049f1f4d-72b8-45e3-afd4-e59e1f9a2758',
    name: 'Maestras de Oaxaca',
    description: 'Tanda mensual del colectivo de profesoras rurales. Para útiles escolares. 📚',
    contribution_amount: 2000,
    frequency: 'monthly',
    max_members: 12,
    current_round: 1,
    total_rounds: 12,
    status: 'active',
    escrow_address: '0x1234567890abcdef1234567890abcdef12345678',
    ai_trust_score: 91,
    created_at: '2026-05-01T08:00:00Z',
    next_payout_at: '2026-06-01T08:00:00Z',
  },
  {
    id: 'ff5dbf1b-23c8-451d-9457-717b1cc44129',
    name: 'Taxistas CDMX',
    description: 'Tanda semanal para mantenimiento vehicular del sitio Reforma. 🚕',
    contribution_amount: 750,
    frequency: 'weekly',
    max_members: 6,
    current_round: 6,
    total_rounds: 6,
    status: 'completed',
    escrow_address: '0xabcdef1234567890abcdef1234567890abcdef12',
    ai_trust_score: 96,
    created_at: '2026-03-01T09:00:00Z',
    next_payout_at: '2026-04-12T09:00:00Z',
  },
  {
    id: '4740134f-4cf5-46a3-a179-daeccbf27c50',
    name: 'Emprendedoras Guadalajara',
    description: 'Nueva tanda para emprendedoras del Mercado San Juan de Dios. ¡Únete! 💪',
    contribution_amount: 1500,
    frequency: 'biweekly',
    max_members: 10,
    current_round: 0,
    total_rounds: 10,
    status: 'forming',
    escrow_address: null,
    ai_trust_score: 0,
    created_at: '2026-05-10T16:00:00Z',
    next_payout_at: null,
  },
  {
    id: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d',
    name: 'Mariachis de Jalisco',
    description: 'Tanda semanal para trajes y renovación de instrumentos. 🎺',
    contribution_amount: 1200,
    frequency: 'weekly',
    max_members: 12,
    current_round: 5,
    total_rounds: 12,
    status: 'active',
    escrow_address: '0xabc123...',
    ai_trust_score: 89,
    created_at: '2026-04-01T12:00:00Z',
    next_payout_at: '2026-05-20T12:00:00Z',
  },
  {
    id: 'b2c3d4e5-f6a7-4b8c-9d0e-1f2a3b4c5d6e',
    name: 'Cocineras de Mérida',
    description: 'Tanda mensual para insumos de banquetería tradicional. 🍲',
    contribution_amount: 2500,
    frequency: 'monthly',
    max_members: 8,
    current_round: 3,
    total_rounds: 8,
    status: 'active',
    escrow_address: '0xdef456...',
    ai_trust_score: 92,
    created_at: '2026-02-15T09:00:00Z',
    next_payout_at: '2026-05-15T09:00:00Z',
  },
  {
    id: 'c3d4e5f6-a7b8-4c9d-0e1f-2a3b4c5d6e7f',
    name: 'Veterinarios Monterrey',
    description: 'Tanda quincenal para fondo de rescate animal y medicinas. 🐾',
    contribution_amount: 1800,
    frequency: 'biweekly',
    max_members: 6,
    current_round: 10,
    total_rounds: 10,
    status: 'completed',
    escrow_address: '0x789ghi...',
    ai_trust_score: 98,
    created_at: '2026-01-10T11:00:00Z',
    next_payout_at: '2026-05-10T11:00:00Z',
  },
];

// ── Members ─────────────────────────────────────────────────
export const MOCK_MEMBERS: TandaMember[] = [
  {
    id: 'c8e8a51d-4290-459b-9eed-aa55f7084cfd',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    wallet_address: '0x70997970C51812dc3A010C7d01b50e0d17dc79C8',
    display_name: 'María González',
    avatar_url: null,
    trust_score: 96,
    payout_position: 1,
    is_current_recipient: false,
    total_contributed: 2000,
    total_received: 5000,
    joined_at: '2026-04-15T10:05:00Z',
  },
  {
    id: 'bbc5962e-62a8-48e8-a787-309e3c09ad10',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    wallet_address: '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
    display_name: 'Lupita Hernández',
    avatar_url: null,
    trust_score: 92,
    payout_position: 2,
    is_current_recipient: false,
    total_contributed: 2000,
    total_received: 5000,
    joined_at: '2026-04-15T10:10:00Z',
  },
  {
    id: 'b306334f-4ee5-4935-ab09-ba3a187d81a7',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    wallet_address: '0x90F79bf6EB2c4f870365E785982E1f101E93b906',
    display_name: 'Rosa Martínez',
    avatar_url: null,
    trust_score: 88,
    payout_position: 3,
    is_current_recipient: false,
    total_contributed: 2000,
    total_received: 0,
    joined_at: '2026-04-15T10:15:00Z',
  },
  {
    id: '6bee417c-aa6f-4317-8e26-2a496016244c',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    wallet_address: '0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65',
    display_name: 'Ana Juárez',
    avatar_url: null,
    trust_score: 85,
    payout_position: 4,
    is_current_recipient: true,
    total_contributed: 2000,
    total_received: 0,
    joined_at: '2026-04-15T10:20:00Z',
  },
  {
    id: '6f4701a1-2390-4e4f-83db-fede2488ecad',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    wallet_address: '0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc',
    display_name: 'Carmen Díaz',
    avatar_url: null,
    trust_score: 79,
    payout_position: 5,
    is_current_recipient: false,
    total_contributed: 2000,
    total_received: 0,
    joined_at: '2026-04-15T10:25:00Z',
  },
  {
    id: 'e1f2a3b4-c5d6-4e7f-8f9d-0a1b2c3d4e5f',
    tanda_id: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d',
    wallet_address: '0x1111111111111111111111111111111111111111',
    display_name: 'Juan Pérez',
    avatar_url: null,
    trust_score: 94,
    payout_position: 1,
    is_current_recipient: false,
    total_contributed: 6000,
    total_received: 14400,
    joined_at: '2026-04-01T12:05:00Z',
  },
  {
    id: 'f2a3b4c5-d6e7-4f8e-9d0c-1a2b3c4d5e6f',
    tanda_id: 'a1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d',
    wallet_address: '0x2222222222222222222222222222222222222222',
    display_name: 'Elena Robles',
    avatar_url: null,
    trust_score: 91,
    payout_position: 2,
    is_current_recipient: false,
    total_contributed: 6000,
    total_received: 14400,
    joined_at: '2026-04-01T12:10:00Z',
  },
];

// ── Recent Contributions ────────────────────────────────────
export const MOCK_CONTRIBUTIONS: Contribution[] = [
  {
    id: '73542108-11ee-4b9f-af6f-86eb03c7ece0',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    member_id: 'c8e8a51d-4290-459b-9eed-aa55f7084cfd',
    round: 4,
    amount: 500,
    currency: 'MXNB',
    status: 'confirmed',
    bitso_tx_id: 'btx_a1b2c3d4',
    botchain_tx_hash: '0xf4a2b1c3d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1',
    created_at: '2026-05-11T08:30:00Z',
    confirmed_at: '2026-05-11T08:30:45Z',
  },
  {
    id: '5f22b3cd-8455-451f-a59c-8774191a8724',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    member_id: 'bbc5962e-62a8-48e8-a787-309e3c09ad10',
    round: 4,
    amount: 500,
    currency: 'MXNB',
    status: 'confirmed',
    bitso_tx_id: 'btx_e5f6g7h8',
    botchain_tx_hash: '0xa1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1',
    created_at: '2026-05-11T09:15:00Z',
    confirmed_at: '2026-05-11T09:15:32Z',
  },
  {
    id: '72185759-52b8-4111-afe3-3a1cb6409d36',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    member_id: 'b306334f-4ee5-4935-ab09-ba3a187d81a7',
    round: 4,
    amount: 500,
    currency: 'MXNB',
    status: 'confirmed',
    bitso_tx_id: 'btx_i9j0k1l2',
    botchain_tx_hash: '0xb2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2',
    created_at: '2026-05-11T10:00:00Z',
    confirmed_at: '2026-05-11T10:00:28Z',
  },
  {
    id: '86a8c502-e2d8-4d28-9b02-3105073729eb',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    member_id: '6bee417c-aa6f-4317-8e26-2a496016244c',
    round: 4,
    amount: 500,
    currency: 'MXNB',
    status: 'pending',
    bitso_tx_id: null,
    botchain_tx_hash: null,
    created_at: '2026-05-13T14:00:00Z',
    confirmed_at: null,
  },
  {
    id: '545b1a05-6f82-4a1b-a2b7-a043ffa60dd4',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    member_id: '6f4701a1-2390-4e4f-83db-fede2488ecad',
    round: 4,
    amount: 500,
    currency: 'MXNB',
    status: 'pending',
    bitso_tx_id: null,
    botchain_tx_hash: null,
    created_at: '2026-05-13T14:00:00Z',
    confirmed_at: null,
  },
];

// ── Recent Payouts ──────────────────────────────────────────
export const MOCK_PAYOUTS: Payout[] = [
  {
    id: '0ecd3ea0-7b7e-402d-a722-86b4006fa4c4',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    recipient_id: 'c8e8a51d-4290-459b-9eed-aa55f7084cfd',
    round: 1,
    amount: 5000,
    currency: 'MXNB',
    status: 'completed',
    bitso_payout_id: 'bpo_x1y2z3',
    botchain_tx_hash: '0xc3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3',
    created_at: '2026-04-22T10:00:00Z',
    completed_at: '2026-04-22T10:01:15Z',
  },
  {
    id: '88a2effe-95c0-4276-8084-9d8da9c98d2d',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    recipient_id: 'bbc5962e-62a8-48e8-a787-309e3c09ad10',
    round: 2,
    amount: 5000,
    currency: 'MXNB',
    status: 'completed',
    bitso_payout_id: 'bpo_a4b5c6',
    botchain_tx_hash: '0xd4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4',
    created_at: '2026-04-29T10:00:00Z',
    completed_at: '2026-04-29T10:00:52Z',
  },
  {
    id: '3a62b559-a92b-44fd-b148-2463068066e5',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    recipient_id: 'b306334f-4ee5-4935-ab09-ba3a187d81a7',
    round: 3,
    amount: 5000,
    currency: 'MXNB',
    status: 'completed',
    bitso_payout_id: 'bpo_d7e8f9',
    botchain_tx_hash: '0xe5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5',
    created_at: '2026-05-06T10:00:00Z',
    completed_at: '2026-05-06T10:01:03Z',
  },
  {
    id: '65e31904-4a93-412e-a0df-9c0fbad611fe',
    tanda_id: 'd41f5312-214e-4030-8047-1a7743bcbc39',
    recipient_id: '6bee417c-aa6f-4317-8e26-2a496016244c',
    round: 4,
    amount: 5000,
    currency: 'MXNB',
    status: 'scheduled',
    bitso_payout_id: null,
    botchain_tx_hash: null,
    created_at: '2026-05-13T10:00:00Z',
    completed_at: null,
  },
];
