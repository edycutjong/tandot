...
export function AITrustList({ members }: { members: TrustMember[] }) {
  const { locale } = useLocale();
  const isEs = locale === 'es';

  const sortedMembers = [...members].sort((a, b) => b.trust_score - a.trust_score);
    ...
    <div>
      ...
      {sortedMembers.length === 0 ? (
        <GlassCard className="p-10 text-center text-(--text-low)">
          {isEs ? 'Aún no hay miembros evaluados.' : 'No scored members yet.'}
        </GlassCard>
      ) : (
