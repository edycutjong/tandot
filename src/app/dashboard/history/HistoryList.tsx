...
import { formatMXNB, botScanUrl } from '@/lib/constants';
...
export function HistoryList({ contributions }: { contributions: HistoryRow[] }) {
  ...
  return (
    <div>
      ...
      {contributions.length === 0 ? (
        <GlassCard className="p-10 text-center text-(--text-low)">
          {isEs ? 'Aún no hay contribuciones registradas.' : 'No contributions recorded yet.'}
        </GlassCard>
      ) : (
                  ...
                  <p className="text-xs text-(--text-low) font-mono">
                    ...
                    {tx.botchain_tx_hash && (
                      <>
                        {' · '}
                        <a
                          href={botScanUrl(`tx/${tx.botchain_tx_hash}`)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-(--cyan-400) hover:underline"
                        >
                          {tx.botchain_tx_hash.slice(0, 10)}… ↗
                        </a>
                      </>
                    )}
                  </p>
